...
public class DrawNormals : MonoBehaviour
{
    ...
    [SerializeField]
    private NormalsDrawData _vertexTangents = new NormalsDrawData(new Color32(255, 200, 255, 127), false);
    ...
    private void OnDrawNormals(bool isSelected)
    {
        ...
        if (_vertexTangents.CanDraw(isSelected))
        {
            ...
            for (int i = 0; i < vertices.Length; i++)
            {
                _vertexTangents.Draw(transform.TransformPoint(vertices[i]), transform.TransformVector(new Vector3(tangents[i].x, tangents[i].y, tangents[i].z)));
            }
        }
    }
#endif
